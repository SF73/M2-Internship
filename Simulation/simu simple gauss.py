...
#axes[0].plot(taueffs/taueffTh,'.',label=r'$\tau_{effTh}/\tau_{effFit}$')
#axes[0].set_title(r'$\tau_{effTh}/\tau_{effFit}$')
#
#idx=np.random.randint(0,len(taueffTh))
#axes[1].bar(Gamths[idx],Aths[idx]/np.sum(Aths[idx]),width=0.1)
#up = max(Aths[idx]/np.sum(Aths[idx]))
#axes[1].bar([1/tau1[idx],1/tau2[idx]],[A1s[idx]/(A1s[idx]+A2s[idx])*up,A2s[idx]/(A1s[idx]+A2s[idx])*up],width=0.1,color='r')
#axes[1].bar(1/taueffs[idx],up,width=0.1)
#axes[1].bar(1/taueffTh[idx],up,width=0.1)
#axes[1].set_title(r'Example (Gamma space)')
#axes[0].set_xlabel(r"Simulation number")
...
taueffs = (A1s*tau1+A2s*tau2)/(A1s+A2s)
axes[0].plot(taueffs/taueffTh,'.',label=r'$\tau_{effTh}/\tau_{effFit}$',alpha=0.5)
axes[0].set_title(r'$\tau_{effTh}/\tau_{effFit}$')
axes[0].set_xlabel(r"Simulation number")
idx=np.random.randint(0,len(taueffTh))
axes[1].bar(Gamths[idx],Aths[idx]/np.sum(Aths[idx]),width=0.1)
up = max(Aths[idx]/np.sum(Aths[idx]))
axes[1].bar([1/tau1[idx],1/tau2[idx]],[A1s[idx]/(A1s[idx]+A2s[idx])*up,A2s[idx]/(A1s[idx]+A2s[idx])*up],width=0.15,color='r')
axes[1].bar(1/taueffs[idx],up,width=0.15)
axes[1].bar(1/taueffTh[idx],up,width=0.15)
axes[1].set_xlabel(r"$\Gamma$")
axes[1].set_title(r'Example (Gamma space)')
fig.tight_layout()
